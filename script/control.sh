...
case "$1" in
        start)
                ...
                STALE_WORKER=$(ps aux | grep "$UNICORN_CFG" | grep worker)
                if [ "$OLD_PID" != "" ]; then
                    echo >&2 "$PREFIX Unicorn is already running (Master PID $OLD_PID)" && exit 0
                elif [ "$STALE_WORKER" != "" ]; then
                    echo >&2 "$PREFIX There's a running unicorn in the wild (workers w/o a master), please check your machine" && exit 0
                else
                    ...
                    if [ "$SUCCESS" -ne 1 ]; then 
                        echo >&2 "$PREFIX Something went wrong. Try doing it manually" && exit 0
                    else
        ...
        stop)
                ...
                if [ "$OLD_PID" != "" ]; then
                    ...
                    if [ "$SUCCESS" -ne 1 ]; then
                        echo >&2 "$PREFIX Couldn't stop unicorn. Try cap deploy:stop and deploy:start" && exit 0
                    else
                        [ -f $PID_FILE ] && rm $PID_FILE
        ...
        restart) # good to have $2 = environment for case where there's no unicorn up
                ...
                if [ "$OLD_PID" != "" ]; then
                    ...
                    if [ "$SUCCESS" -ne 1 ]; then
                        echo >&2 "$PREFIX Something went wrong. Try cap deploy:stop and deploy:start" && exit 0
                    else
                        MASTER_PID=$(ps aux | grep "$UNICORN_CFG" | grep master | awk '{print $2}')
